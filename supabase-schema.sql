...
--
-- Name: user_stats Allow anyone to read any user's stats; Type: POLICY; Schema: public; Owner: supabase_admin
--

CREATE POLICY "Allow anyone to read any user's stats" ON public.user_stats FOR SELECT USING (true);
...
CREATE POLICY "Users can manage own queue row" ON public.matchmaking_queue USING ((auth.uid() = uid)) WITH CHECK ((auth.uid() = uid));
...
CREATE POLICY "Users can read own matches" ON public.matches FOR SELECT USING (((auth.uid() = player1_uid) OR (auth.uid() = player2_uid)));
...
CREATE POLICY "Users can upsert own heartbeat" ON public.matchmaking_heartbeats USING ((auth.uid() = uid)) WITH CHECK ((auth.uid() = uid));
...
GRANT ALL ON TABLE public.matchmaking_heartbeats TO anon;
