...
void main() {
  ...
  for (float i = 0.0; i < 120.0; i++) {
    ...
    float brightness = smoothstep(size * size, 0.0, dist2) * (0.55 + hash(seed + 2.0) * 0.75);
  ...
  for (float i = 0.0; i < 100.0; i++) {
    ...
    float tSpeed = time * speed * 0.39;
    float x = fract(phase + tSpeed) * 2.9 - 1.45;
    ...
    float brightness = smoothstep(thickness * thickness * 9.0, 0.0, dist2)
                     * smoothstep(0.0, 0.34, depth)
                     * smoothstep(1.0, 0.36, depth);
    brightness *= 0.9 + hash(seed+4.0) * 1.3;

    float colorType = hash(seed + 0.7);
    vec3 starColor = (colorType < 0.33) ? vec3(1.0, 0.57, 0.71) :
                     (colorType < 0.66) ? vec3(0.57, 0.81, 1.0) : vec3(0.96, 0.98, 1.0);

    color += starColor * brightness * (1.1 + speed * 0.38);
  }

  float vignette = 1.0 - smoothstep(0.6, 2.2, dot(uv, uv));
  color = color*vignette * 0.97 + vec3(0.12, 0.17, 0.27) * (1.0 - vertDist) * 0.5;
